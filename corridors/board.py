...
class Piece:
    ...
    def hasWon(self):
        j = self.location[0]
        return ((self.color == "red") and (j == 0)) or (
            (self.color == "blue") and (j == self.N - 1)
        )

    def __json__(self):
        return {"color": self.color, "location": self.location, "walls": self.walls}
...
class Board:
    ...
    def __json__(self):
        return {
            "red": self.red,
            "blue": self.blue,
            "walls": [[int(cell) for cell in row] for row in self.walls],
            # "history":self.history,
            "turn": self.turn,
            "settings": {"N": self.N, "walls_per_piece": self.wallsPerPiece},
        }
    ...
    def info(self):
        ...
        return """
            ...
            {}
        """.format(
            self.turn.capitalize(), self.red.walls, self.blue.walls, winBanner
        )
    ...
    def legalMove(self, location, direction, checkPieces=True):
        ...
        if (j == 0) and (direction == "up"):
            return False
        if (j >= M) and (direction == "down"):
            return False
        if (i == 0) and (direction == "left"):
            return False
        if (i >= M) and (direction == "right"):
            return False
        ...
        if "up" == direction:
            if i > 0:
                if self.walls[(j - 1, i - 1)] == HORIZONTAL:
                    return False
            if i < M:
                if self.walls[(j - 1, i)] == HORIZONTAL:
                    return False

        if "left" == direction:
            if j > 0:
                if self.walls[(j - 1, i - 1)] == VERTICAL:
                    return False
            if j < M:
                if self.walls[(j, i - 1)] == VERTICAL:
                    return False

        if "right" == direction:
            if j > 0:
                if self.walls[(j - 1, i)] == VERTICAL:
                    return False
            if j < M:
                if self.walls[(j, i)] == VERTICAL:
                    return False

        if "down" == direction:
            if i > 0:
                if self.walls[(j, i - 1)] == HORIZONTAL:
                    return False
            if i < M:
                if self.walls[(j, i)] == HORIZONTAL:
                    return False
    ...
    def piecesCanEscape(self):
        return self.canEscape(self.red) and self.canEscape(self.blue)
    ...
    def _addWall(self, location, orientation):
        ...
        if self.do_checks:
            ...
            ruleCheck(
                self.legalWall(location, orientation),
                "illegal wall at {}: {}".format(location, orientation),
            )
            ruleCheck(
                self.escapableWall(location, orientation),
                "trapping wall at {}: {}".format(location, orientation),
            )
